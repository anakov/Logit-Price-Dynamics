...
%    Called by gess.m AFTER GE_ss.mat has been reloaded in to memory.
%    So all objects in GE_ss.mat are in memory when calcstats.m starts.
...
checkEPchange = Pchangegrid'*masspchanges/freqnonzeropchanges;
...
%disp('CHECK MEAN P CHANGE: ')
%[EPchange checkEPchange  EPincrease*fracPriceIncr+EPdecrease*fracPriceDecr]
...
AvTimingCosts = wbar*sum(sum(timecost.*Pdisteroded));
AvAdjCosts = wbar*sum(sum(lambda.*adjcost.*Pdisteroded));

Nbar   = Cbar*sum(sum(Pdist.*sMAT.*PMAT.^(-epsilon))) + (AvTimingCosts+AvAdjCosts)/wbar;    % aggregate hours
...
Dcutoff=0.005*MedV;                                                   % very few firms should forgo such huge gains
...
%   Dgrid=linspace(minD,maxD,201);
   ...
   %Lvalues = adjustment(adjtype, (Dgrid-averadjcost)'/wbar, ksi, alpha, lbar);
   %Lvalues = adjustment(adjtype, Dgrid'/wbar, ksi, alpha, lbar);
%end
...
%NEXT TWO LINES INCONSISTENT WITH DYNAMIC SIMULATION:
%[pstarprob, pstarindex]=max(logitprobMAT);
%pstar = Pgrid(pstarindex)';
[Mstar, pstar] = M_pStar(V,Pgrid,mu);
%Pdistans=ones(nump,1)*pstar-Pgrid*ones(1,nums);                 % matrix of distances from optimal price
Pdistans = ones(nump,1)*pstar-logPMAT ;   %EQUIVALENT but using new notation.

%selection = sum(sum(Pdistans.*(lambda - sum(sum(lambda .*Pdisteroded))).*Pdisteroded));
%intensive = sum(sum(Pdistans.*Pdisteroded )) ;              % = desiredadj_ss
%extensive = sum(sum(lambda.*Pdisteroded ))   ;              % = fracadjusters_ss
%MORE LOGICAL ORDER & NAMING:
fracadjusters = sum(sum(lambda.*Pdisteroded ))   ;
desiredadj = sum(sum(Pdistans.*Pdisteroded )) ;
selection = sum(sum(Pdistans.*(lambda - fracadjusters).*Pdisteroded));
desiredinflation = sum(sum(Pdistans.*lambda.*Pdisteroded));

adjprobVEC = sum(lambda.*Pdisteroded) ;  %PROB OF ADJ AS FN OF PRODUCTIVITY
logiterrorMAT = -logitprobMAT.*Pdistans ; %Distance from opt AFTER logit
logiterrorVEC = sum(logiterrorMAT) ;     %Distance from opt AS FN OF PROD
avgerror = sum(logiterrorVEC.*adjprobVEC) ; %STEADY STATE AVERAGE INFLATION ERROR (per period, ie per month)
%format long
%disp('Inflation decomposition: selection, intensive+extensive, total desired, avgerror:')
%[selection fracadjusters*desiredadj desiredinflation  avgerror]
%predictedinflation = exp(desiredinflation+avgerror)-1;
predictedinflation = desiredinflation+avgerror;
%disp('Desired inflation, avg error, desired+error, mean inflation:')
%[desiredinflation  avgerror  predictedinflation  PI_SS]
%format short
...
averprof = sum(sum(grossprofits.*Pdist)) - (AvAdjCosts+AvTimingCosts);
    %- sum(sum(wbar*adjcost.*lambda.*Pdisteroded));
averoptprof = sum(sum(grossprofits.*ergodicDistFlexPrice));
avoptrev = sum(sum(revenue.*ergodicDistFlexPrice));

AdjCostinRev = AvAdjCosts/AvRevenue;
AdjCostinOptRev = AvAdjCosts/avoptrev;
TimingCostinRev = AvTimingCosts/AvRevenue;
TimingCostinOptRev = AvTimingCosts/avoptrev;

loss1 = (averoptprof - averprof)/ averoptprof;
loss2 = (averoptprof - averprof)/ avoptrev;
LossinRev = (averoptprof - averprof)/ AvRevenue;
